...
## ADR-051 — Email invites and reset links over SMTP, token in the URL fragment
Team members can be invited by email and sent password-reset links. SMTP (`SMTP_URL`, `EMAIL_FROM`) rather than one provider's API, so Gmail, Zoho, Brevo, Resend or a host's mail all work. Links are `/invite#<token>`: the fragment is never sent to a server, so the token can't appear in access logs, proxies or Referer headers; the page reads it, removes it from the address bar, and posts it in a request body. Only an HMAC of the token is stored; use is a single conditional UPDATE, so a link works exactly once. Without SMTP the feature says so and the temporary-password path remains; in development with mocks, emails are written to `data/outbox` (labelled mock) so tests can follow the real flow without sending anything. Sending an invite is a direct action by the owner (the click is the approval), not an agent action, so it doesn't go through the Approval Center; it is audited.
