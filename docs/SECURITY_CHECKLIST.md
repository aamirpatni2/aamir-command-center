...
| Emailed invite/reset links: 256-bit single-use tokens, only an HMAC stored, token in the URL fragment (never sent to servers, logs or Referer) and removed from the address bar, 72 h / 1 h expiry, revoked on email change, deactivation, resend or a temporary-password reset; atomic single use; rate-limited public endpoints | ✅ | `invites.test.ts`, `e2e/invites.spec.ts` |
